...
//
// from: SebLague
// github: https://github.com/SebLague/Hydraulic-Erosion/blob/master/Assets/Scripts/ComputeShaders/Erosion.compute
// -----------------------------------------------------
...
StructuredBuffer<int> randomIndices : register(t0);
StructuredBuffer<int> brushIndices : register(t1);
StructuredBuffer<float> brushWeights : register(t2);
RWStructuredBuffer<float> map : register(u0);

// int mapSize = 255;
// int brushLength = 20;
// int borderSize = 10;

// int maxLifetime = 50000;
// float inertia = .05f;
...
// https://learn.microsoft.com/ja-jp/previous-versions/direct-x/ee416048(v=vs.85)?redirectedfrom=MSDN
// https://tositeru.github.io/ImasaraDX11/part/constant-buffer

cbuffer ERODE_SETTINGS : register(b0) {
    int mapSize;
    int brushLength;
    int borderSize;
...
void CSMain (uint3 id : SV_DispatchThreadID)
{
    ...
    //map[id.x] = brushWeights[id.x % borderSize];
    //map[id.x] = brushWeights[id.x % borderSize];
    //return;
